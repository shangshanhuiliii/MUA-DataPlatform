...
class DeviceState(object):
    ...
    def __init__(self, width, height, views, foreground_activity, activity_stack, background_services,
                 tag=None, screenshot_path=None, use_image_state=False,
                 logger=None, output_dir=None, humanoid=None, state_str=None, structure_str=None):
        # New attributes
        ...
        if tag is None:
            now = datetime.now()
            tag = now.strftime("%Y-%m-%d_%H%M%S") + f"_{now.microsecond:06d}"
        self.tag = tag
        ...
        self.structure_str = structure_str if structure_str is not None else self.__get_content_free_state_str()
    ...
    @property
    def state_json_path(self):
        return os.path.join(self.output_dir,  f"states/state_{self.tag}.json")
    ...
    def __get_state_str(self):
        if hasattr(self, 'state_str') and self.state_str is not None:
            return self.state_str
        state_str_raw = self.__get_state_str_raw()
        return md5(state_str_raw)

    def __get_state_str_raw(self):
        # Check if image-based state generation is enabled
        if self.use_image_state:
            try:    
                import base64
                # Read screenshot as binary and convert to base64
                with open(self.screenshot_path, 'rb') as img_file:
                    img_data = img_file.read()
                    img_base64 = base64.b64encode(img_data).decode('utf-8')
                    # Combine with activity for better uniqueness
                    return img_base64
            except Exception as e:
                self.logger.warning(f"Failed to generate image-based state_str: {e}")
                raise e

        # Traditional methods (humanoid or view-based)
    ...
    def set_image_state_mode(self, enabled=True):
        """
        Enable or disable image-based state_str generation
        :param enabled: bool, whether to use image-based state generation
        """
        old_mode = self.use_image_state
        self.use_image_state = enabled
        
        # Regenerate state_str if mode changed
        if old_mode != enabled:
            self.state_str = self.__get_state_str()

    def __get_content_free_state_str(self):
        if hasattr(self, 'structure_str') and self.structure_str is not None:
    ...
    def save2dir(self, states_dir=None):
        try:
            ...
            dest_state_json_path = self.state_json_path
            
            # Determine target file extension based on original screenshot extension
            if self.screenshot_path:
                from pathlib import Path
                file_ext = Path(self.screenshot_path).suffix or '.png'
            else:
                file_ext = '.png'
                
            dest_screenshot_path = "%s/screen_%s%s" % (states_dir, self.tag, file_ext)
    ...
    @classmethod
    def load_from_file(cls, state_json_path, screenshot_path=None, output_dir=None, logger=None):
        """
        从状态JSON文件加载DeviceState对象
        
        :param state_json_path: 状态JSON文件路径
        :param screenshot_path: 截图文件路径，如果未指定则自动查找
        :param output_dir: 输出目录，如果未指定则使用state_json_path.parent.parent
        :param logger: 可选的日志记录器
        :return: 重构的DeviceState对象
        """
        logger = logger or logging.getLogger(cls.__name__)
        
        if not os.path.exists(state_json_path):
            raise FileNotFoundError(f"State JSON file not found: {state_json_path}")

        state_dir = Path(state_json_path).parent
        # 设置默认output_dir
        if output_dir is None:
            output_dir = str(state_dir.parent)
        ...
        if screenshot_path is None:
            ...
            for ext in ['.png', '.jpg', '.jpeg']:
                screenshot_name = f"screen_{tag}{ext}"
                potential_path = state_dir / screenshot_name
                if potential_path.exists():
                    screenshot_path = str(potential_path)
                    break

        # 从JSON数据重构DeviceState对象
        state = cls(
            width=state_data['width'],
            height=state_data['height'],
            views=state_data['views'],
            foreground_activity=state_data['foreground_activity'],
            activity_stack=state_data['activity_stack'],
            background_services=state_data['background_services'],
            tag=state_data['tag'],
            screenshot_path=screenshot_path,
            use_image_state=state_data.get('use_image_state', False),
            logger=logger,
            output_dir=output_dir,
            state_str=state_data.get('state_str'),
            structure_str=state_data.get('state_str_content_free')
        )
        ...
        try:
            if output_dir is None:
                if self.output_dir is None:
                    ...
                    output_dir = os.path.join(self.output_dir, "views")
            if not os.path.exists(output_dir):
                os.makedirs(output_dir)
            view_str = view_dict['view_str']
            
            # Determine view image extension based on original screenshot extension
            if self.screenshot_path:
                from pathlib import Path
                file_ext = Path(self.screenshot_path).suffix or '.png'
            else:
            ...
            view_file_path = "%s/view_%s%s" % (output_dir, view_str, file_ext)
