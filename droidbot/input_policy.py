...
from .input_event import InputEvent, KeyEvent, IntentEvent, TouchEvent, ManualEvent, SetTextEvent, PutTextEvent, KillAppEvent
from .utg import UTG
...
POLICY_NAIVE_DFS = PolicyType.NAIVE_DFS.value
POLICY_GREEDY_DFS = PolicyType.GREEDY_DFS.value
POLICY_NAIVE_BFS = PolicyType.NAIVE_BFS.value
POLICY_GREEDY_BFS = PolicyType.GREEDY_BFS.value
POLICY_REPLAY = PolicyType.REPLAY.value
POLICY_MANUAL = PolicyType.MANUAL.value
POLICY_MONKEY = PolicyType.MONKEY.value
POLICY_NONE = PolicyType.NONE.value
POLICY_MEMORY_GUIDED = PolicyType.MEMORY_GUIDED.value
POLICY_LLM_GUIDED = PolicyType.LLM_GUIDED.value
    ...
    def start(self, input_manager):
        ...
        while input_manager.enabled and self.action_count < input_manager.event_count:
            try:
                ...
                if self.action_count == 0 and self.master is None:
                    # Skip KillAppEvent for DummyApp since there's no real app to kill
                    if isinstance(self.app, DummyApp):
                        self.logger.info("Skipping KillAppEvent for DummyApp - no real app to kill")
...
class UtgBasedInputPolicy(InputPolicy):
    ...
    def __init__(self, device, app, random_input):
        ...
        self.utg = UTG(
            ...
            device_model_number=device.get_model_number(),
            device_sdk_version=device.get_sdk_version(),
            app_signature=app.signature,
            app_package=app.package_name,
            app_main_activity=app.main_activity,
            app_num_total_activities=len(app.activities),
...
class UtgNaiveSearchPolicy(UtgBasedInputPolicy):
    ...
    def select_a_view(self, state):
        ...
        if self.search_method == PolicyType.NAIVE_DFS.value:
            views.append(mock_view_back)
        elif self.search_method == PolicyType.NAIVE_BFS.value:
...
class UtgGreedySearchPolicy(UtgBasedInputPolicy):
    ...
    def generate_event_based_on_utg(self):
        ...
        if self.search_method == PolicyType.GREEDY_DFS.value:
            possible_events.append(KeyEvent(name="BACK"))
        elif self.search_method == PolicyType.GREEDY_BFS.value:
...
class ManualPolicy(UtgBasedInputPolicy):
    ...
    def __init__(self, device, app):
        ...
        self.__is_dummy_app = isinstance(app, DummyApp)
    ...
    def generate_event_based_on_utg(self):
        ...
        @return: InputEvent
        """
        if self.__first_event and not self.__is_dummy_app:
            self.__first_event = False
            self.logger.info("Trying to start the app...")
            start_app_intent = self.app.get_start_intent()
            return IntentEvent(intent=start_app_intent)
        else:
            if self.__is_dummy_app and self.__first_event:
                self.__first_event = False
                self.logger.info("Manual mode without specific app. Ready for manual interaction...")
            keyboard_input = Prompt.ask("press ENTER to save current state, type q to exit...", default="", show_default=False)
            if keyboard_input.strip().lower() == "q":
