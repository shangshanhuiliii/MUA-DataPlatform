...
from .adapter.uiautomator2_conn import UIAutomator2AppConn
...
from .constant import ViewsMode
...
class Device(object):
    ...
    def __init__(self, device_serial=None, is_emulator=False, output_dir=None,
                 views_mode=ViewsMode.XML_MODE, grant_perm=False, telnet_auth_token=None,
        ...
        self.droidbot_app = UIAutomator2AppConn(device=self)
    ...
    def unlock(self):
        ...
        if self.droidbot_app and self.adapters[self.droidbot_app]:
            try:
                if self.droidbot_app.unlock():
                    return
                else:
                    self.logger.warning("Failed to unlock using UIAutomator2")
            except Exception as e:
                self.logger.warning("UIAutomator2 unlock failed: %s" % e)
    ...
    def start_app(self, app):
        ...
        if isinstance(app, str):
            package_name = app
        elif isinstance(app, (App, InstalledApp)):
            package_name = app.get_package_name()
    ...
    def take_screenshot(self):
        ...
        # Use UIAutomator2AppConn for screenshot
        if self.droidbot_app and self.adapters[self.droidbot_app]:
            try:
                local_image_path = os.path.join(local_image_dir, "screen_%s.png" % tag)
                result = self.droidbot_app.get_screenshot(local_image_path)
                if result:
                    return local_image_path
                else:
                    self.logger.warning("Failed to take screenshot using UIAutomator2")
            except Exception as e:
                self.logger.warning("UIAutomator2 screenshot failed: %s" % e)
    ...
    def get_current_state(self):
        ...
        try:
            ...
            t1 = time.time()
            self.logger.info(f"⏱️ get_views() took {t1 - t0:.2f}s")

            foreground_activity = self.get_top_activity_name()
            t2 = time.time()
            self.logger.debug(f"get_top_activity_name() took {t2 - t1:.2f}s")

            activity_stack = self.get_current_activity_stack()
            t3 = time.time()
            self.logger.debug(f"get_current_activity_stack() took {t3 - t2:.2f}s")

            background_services = self.get_service_names()
            t4 = time.time()
            self.logger.debug(f"get_service_names() took {t4 - t3:.2f}s")

            screenshot_path = self.take_screenshot()
            t5 = time.time()
            self.logger.info(f"⏱️ take_screenshot() took {t5 - t4:.2f}s")
            ...
            t6 = time.time()
            self.logger.debug(f"DeviceState creation took {t6 - t5:.2f}s")
        ...
        self.logger.info(f"⏱️ get_current_state total took {t_total:.2f}s")
    ...
    def view_touch(self, x, y):
        if self.droidbot_app and self.adapters[self.droidbot_app]:
            try:
                if self.droidbot_app.touch(x, y):
                    return
                else:
                    self.logger.warning("Failed to touch using UIAutomator2")
            except Exception as e:
                self.logger.warning("UIAutomator2 touch failed: %s" % e)
    ...
    def view_long_touch(self, x, y, duration=2000):
        ...
        if self.droidbot_app and self.adapters[self.droidbot_app]:
            try:
                if self.droidbot_app.long_touch(x, y, duration):
                    return
                else:
                    self.logger.warning("Failed to long touch using UIAutomator2")
            except Exception as e:
                self.logger.warning("UIAutomator2 long touch failed: %s" % e)
    ...
    def view_drag(self, start_xy, end_xy, duration):
        ...
        if self.droidbot_app and self.adapters[self.droidbot_app]:
            try:
                if self.droidbot_app.drag(start_xy, end_xy, duration):
                    return
                else:
                    self.logger.warning("Failed to drag using UIAutomator2")
            except Exception as e:
                self.logger.warning("UIAutomator2 drag failed: %s" % e)
    ...
    def view_append_text(self, text):
        if self.droidbot_app and self.adapters[self.droidbot_app]:
            try:
                if self.droidbot_app.input_text(text=text, mode=1):
                    return
                else:
                    self.logger.warning("Failed to append text using UIAutomator2")
                    self.adb.type(text)
            except Exception as e:
                self.logger.warning("UIAutomator2 text input failed: %s" % e)
        else:
            self.adb.type(text)
    ...
    def view_set_text(self, text):
        if self.droidbot_app and self.adapters[self.droidbot_app]:
            try:
                if self.droidbot_app.input_text(text=text, mode=0):
                    return
                else:
                    self.logger.warning("Failed to set text using UIAutomator2")
    ...
    def key_press(self, key_code):
        if self.droidbot_app and self.adapters[self.droidbot_app]:
            try:
                if self.droidbot_app.press_key(key_code):
                    return
                else:
                    self.logger.warning("Failed to press key using UIAutomator2")
            except Exception as e:
                self.logger.warning("UIAutomator2 key press failed: %s" % e)
    ...
    def get_views(self):
        if self.views_mode == ViewsMode.CV_MODE:
            if self.minicap and self.adapters[self.minicap]:
                # Get views using cv module
                views = self.minicap.get_views()
            elif self.droidbot_app and self.adapters[self.droidbot_app]:
                views = self.droidbot_app.get_views_cv_mode()
            else:
                views = None
        ...
        elif self.views_mode == ViewsMode.YOLO_MODE:
            views = self.droidbot_app.get_views_yolo_mode()
            if views:
                return views
            else:
                self.logger.warning("Failed to get views using UIYolo.")
    ...
    def get_view_by_coordinates(self, x, y, views=None, is_enabled=None, is_visible=None,
                               is_focusable=None, is_editable=None, is_clickable=None, 
                               is_scrollable=None, is_checkable=None, is_long_clickable=None):
        """
        Find views that contain the specified coordinates (x, y) with optional filtering
        :param x: x coordinate
        :param y: y coordinate  
        :param views: optional list of views to search in, if None will get current views
        :param is_enabled: if True, only return enabled views; if False, only disabled views; if None, no filter
        :param is_visible: if True, only return visible views; if False, only invisible views; if None, no filter
        :param is_clickable: if True, only return clickable views; if False, only non-clickable views; if None, no filter
        :param is_scrollable: if True, only return scrollable views; if False, only non-scrollable views; if None, no filter
        :param is_focusable: if True, only return focusable views; if False, only non-focusable views; if None, no filter
        :param is_editable: if True, only return editable views; if False, only non-editable views; if None, no filter
        :param is_checkable: if True, only return checkable views; if False, only non-checkable views; if None, no filter
        :param is_long_clickable: if True, only return long clickable views; if False, only non-long clickable views; if None, no filter
        :return: list of views containing the coordinates that match ALL filter conditions, ordered from smallest to largest area
        """
        if views is None:
            views = self.get_views()
        
        if not views:
            self.logger.warning("No views available to search")
            return []
        
        matching_views = []
        
        for view in views:
            if not isinstance(view, dict) or 'bounds' not in view:
                continue
            ...
            # Check if coordinates are within bounds (inclusive)
            if x1 <= x <= x2 and y1 <= y <= y2:            

                # Apply all other filters with AND logic - all conditions must be satisfied
                if is_visible is not None and view.get('visible', False) != is_visible:
                    continue
                if is_clickable is not None and view.get('clickable', False) != is_clickable:
                    continue
                if is_scrollable is not None and view.get('scrollable', False) != is_scrollable:
                    continue
                if is_focusable is not None and view.get('focusable', False) != is_focusable:
                    continue
                if is_editable is not None and view.get('editable', False) != is_editable:
                    continue
                if is_enabled is not None and view.get('enabled', True) != is_enabled:
                    continue
                if is_checkable is not None and view.get('checkable', False) != is_checkable:
                    continue
                if is_long_clickable is not None and view.get('long_clickable', False) != is_long_clickable:
                    continue
                
                matching_views.append(view)
        
        # Sort by area (smallest first) - most specific views first
        matching_views.sort(key=lambda v: abs(v['bounds'][1][0] - v['bounds'][0][0]) * 
                                         abs(v['bounds'][1][1] - v['bounds'][0][1]))
