...
DEFAULT_POLICY = PolicyType.GREEDY_DFS.value
...
class InputManager(object):
    ...
    def get_input_policy(self, device, app, master):
        if self.policy_name == PolicyType.NONE.value:
            input_policy = None
        elif self.policy_name == PolicyType.MONKEY.value:
            input_policy = None
        elif self.policy_name in [PolicyType.NAIVE_DFS.value, PolicyType.NAIVE_BFS.value]:
            input_policy = UtgNaiveSearchPolicy(device, app, self.random_input, self.policy_name)
        elif self.policy_name in [PolicyType.GREEDY_DFS.value, PolicyType.GREEDY_BFS.value]:
            input_policy = UtgGreedySearchPolicy(device, app, self.random_input, self.policy_name)
        elif self.policy_name == PolicyType.MEMORY_GUIDED.value:
            from .input_policy2 import MemoryGuidedPolicy
            input_policy = MemoryGuidedPolicy(device, app, self.random_input)
        elif self.policy_name == PolicyType.LLM_GUIDED.value:
            from .input_policy3 import LLM_Guided_Policy
            input_policy = LLM_Guided_Policy(device, app, self.random_input)
        elif self.policy_name == PolicyType.REPLAY.value:
            input_policy = UtgReplayPolicy(device, app, self.replay_output)
        elif self.policy_name == PolicyType.MANUAL.value:
    ...
    def start(self):
        ...
        try:
            if self.policy is not None:
            ...
            elif self.policy_name == PolicyType.NONE.value:
                ...
                throttle = int(self.event_interval * 1000)
