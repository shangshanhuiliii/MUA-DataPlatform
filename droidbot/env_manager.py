...
# Environment policy taxonomy
class EnvPolicyType(Enum):
    NONE = "none"
    DUMMY = "dummy"
    STATIC = "static"

# Backward compatibility constants (deprecated)
POLICY_NONE = EnvPolicyType.NONE.value
POLICY_DUMMY = EnvPolicyType.DUMMY.value
POLICY_STATIC = EnvPolicyType.STATIC.value

DEFAULT_POLICY = EnvPolicyType.NONE.value
...
class DummyFilesEnv(StaticAppEnv):
    ...
    def __init__(self, dummy_files_dir=None):
        ...
        if dummy_files_dir is None:
            from .resource_utils import get_droidbot_resource_dir
            dummy_files_dir = get_droidbot_resource_dir("dummy_documents")
...
class AppEnvManager(object):
    ...
    def __init__(self, device, app, env_policy):
        ...
        if not self.policy:
            self.policy = EnvPolicyType.NONE.value

        if self.policy == EnvPolicyType.NONE.value:
            self.env_factory = None
        elif self.policy == EnvPolicyType.DUMMY.value:
            self.env_factory = DummyEnvFactory()
        elif self.policy == EnvPolicyType.STATIC.value:
